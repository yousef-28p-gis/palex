...
function UserDisputesContent() {
  ...
  const loadDisputes = async () => {
    setIsLoading(true);
    try {
      const res = await disputesApi.getUserDisputes(page);
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white">نزاعاتي</h1>
              <p className="text-blue-200 text-sm mt-1">عرض ومتابعة النزاعات الخاصة بك</p>
            </div>
          </div>
          
          <div className="flex gap-3">
            <button onClick={loadDisputes} className="p-2 rounded-xl bg-white/10 border border-white/20 text-white hover:bg-white/20 transition" title="تحديث">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="بحث برقم الصفقة أو سبب النزاع..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full md:w-80 pr-10 pl-4 py-2 rounded-xl bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
              />
            </div>
          </div>
        </div>

        {/* Stats Cards - نفس تصميم الأدمن */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-5 border border-white/20">
            <p className="text-blue-200 text-sm mb-1">إجمالي نزاعاتي</p>
        ...
        {disputes.length === 0 ? (
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-12 text-center border border-white/20">
            ...
            <p className="text-blue-200 text-sm">لم تقم بفتح أي نزاع حتى الآن</p>
          </div>
        ) : (
